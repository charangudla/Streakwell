          ...
          <section className="mt-10">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              {activeList.slice(1).map((uc) => (
                <ActiveCardCompact key={uc.id} uc={uc} />
              ))}
            </div>
          </section>
        ) : null}
...
function ActiveCardCompact({ uc }: { uc: UserChallenge }) {
  const c = uc.challenge;
  ...
  return (
    <Link
      href={`/my-challenges/${uc.id}/checkin`}
      className="group rounded-2xl border border-slate-200 bg-white p-5 transition-all hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-lg"
    >
