...
import { Search, Eye, UserX, UserCheck, Users, BadgeCheck } from 'lucide-react';
import { getUsers, setUserActive } from '../api/service';
...
export function UsersPage() {
  ...
  const loadUsers = useCallback((searchTerm: string) => {
    setIsLoading(true);
    getUsers({ search: searchTerm || undefined, skip: 0, take: PAGE_SIZE })
      .then((data) => {
  ...
  const handleConfirmToggle = async () => {
    try {
      await setUserActive(confirmModal.userId, !confirmModal.isActive);
    ...
    <div className="space-y-6">
                ...
                <tr key={usr.id} className={`hover:bg-slate-50 transition-colors ${usr.isActive ? '' : 'bg-slate-50/35 text-slate-400'}`}>
                  <td className="p-4">
                    <p className="font-bold text-slate-900 flex items-center gap-1.5">
                      {usr.name}
                      {usr.emailVerified && (
                        <BadgeCheck size={14} className="text-emerald-500" aria-label="Email verified" />
                      )}
                    </p>
                  ...
                  <td className="p-4 font-semibold text-slate-500">{usr.challengeCount}</td>
                    ...
                    <div className="flex items-center justify-end gap-3">
                      {usr.role !== 'SUPER_ADMIN' && (
                        <button
                          ...
                          type="button"
                          title={usr.isActive ? 'Suspend User' : 'Reactivate User'}
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to ${
          confirmModal.isActive ? 'SUSPEND' : 'REACTIVATE'
        } user account "${confirmModal.userName}"? Suspended users cannot access the mobile or web app.`}
        confirmText={confirmModal.isActive ? 'Suspend Account' : 'Reactivate Account'}
